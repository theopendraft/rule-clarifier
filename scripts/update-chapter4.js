...
async function updateChapter4() {
  try {
    console.log('Updating Chapter 4 with HTML content...');
    
    // First, ensure system user exists
    const systemUser = await prisma.user.upsert({
      where: { email: 'system@railway.gov' },
      update: {},
      create: {
        email: 'system@railway.gov',
        name: 'System',
        role: 'ADMIN'
      }
    });
    
    // Then, find or create a default RuleBook
    const ruleBook = await prisma.ruleBook.upsert({
      ...
      create: {
        id: 'default-book',
        title: 'General Rules',
        description: 'Default rule book for railway operations',
        createdBy: systemUser.id
      }
    });

    // Now find existing chapter 4 or create new one
    ...
    console.log('Chapter 4 updated successfully with ID:', chapter.id);
    console.log('HTML content stored in database');
